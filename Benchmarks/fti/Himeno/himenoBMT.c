...
jacobi(int nn)
{
  ...
  int num = world_rank;
    FTI_Protect(1,&n,1,FTI_INTG);
    FTI_Protect(2,&num,1,FTI_INTG);
    FTI_Protect(3,p,MIMAX*MJMAX*MKMAX,FTI_SFLT);
    if(FTI_Status()!=0){
        FTI_Recover();
    }
  for(;n<nn;++n){
    ...
    for(i=1 ; i<imax-1 ; ++i)
      for(j=1 ; j<jmax-1 ; ++j)
        for(k=1 ; k<kmax-1 ; ++k)
          p[i][j][k] = wrk2[i][j][k];
    FTI_Snapshot();
    MPI_Allgather(&num,1,MPI_INT,arr,1,MPI_INT,FTI_COMM_WORLD);
    num+=1; 
} /* end n loop */
