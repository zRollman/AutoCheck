...
/*  This is the main iteration */
    ...
    iteration = 1;
    if(restart_id){
	    read_cp(ranks,&iteration,&location,&passed_verification,1 * sizeof(int));
	    read_cp(ranks,&iteration,&location,bucket_ptrs,NUM_BUCKETS * sizeof(INT_TYPE));
	    read_cp(ranks,&iteration,&location,key_array,SIZE_OF_BUFFERS * sizeof(INT_TYPE));
    }
    ...
    for( iteration; iteration<=MAX_ITERATIONS; iteration++ )
    {
        if( CLASS != 'S' ) printf( "        %d\n", iteration );
        rank( iteration );
	write_cp(ranks,iteration,&passed_verification,1 * sizeof(int));
	write_cp(ranks,iteration,bucket_ptrs,NUM_BUCKETS * sizeof(INT_TYPE));
	write_cp(ranks,iteration,key_array,SIZE_OF_BUFFERS * sizeof(INT_TYPE));
	if(iteration == 3)
		exit(0);
    }
