...
void ssor(int niter)
{
  ...
  */
  ...
  istep = 1;
  int cpk_size = ISIZ3 * (ISIZ2/2*2+1) * (ISIZ1/2*2+1);
  if(restart_id){
	  read_cp(rank,&istep,&location,rho_i,cpk_size * sizeof(double));
	  read_cp(rank,&istep,&location,u,5 * cpk_size * sizeof(double));
	  read_cp(rank,&istep,&location,qs,cpk_size * sizeof(double));
	  read_cp(rank,&istep,&location,rsd,5 * cpk_size * sizeof(double));
  }

  for (istep; istep <= niter; istep++) {
    ...
    write_cp(rank,istep,rho_i,cpk_size * sizeof(double));
    write_cp(rank,istep,u,cpk_size * 5 * sizeof(double));
    write_cp(rank,istep,qs,cpk_size * sizeof(double));
    write_cp(rank,istep,rsd,cpk_size * 5 * sizeof(double));
    if(istep == 3)
	    exit(0);	
  }
