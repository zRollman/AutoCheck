...
void appft(int niter, double *total_time, logical *verified)
{
  ...
  kt = 1;
  if(restart_id){
	  read_cp(rank,&kt,&location,sums,(NITER_DEFAULT+1) * sizeof(dcomplex));
	  read_cp(rank,&kt,&location,y,(NX + 1) * NZ * NY * sizeof(dcomplex));
  }
  ...
  for (kt; kt <= niter; kt++) {
    ...
    write_cp(rank,kt,sums,(NITER_DEFAULT+1) * sizeof(dcomplex));
    write_cp(rank,kt,y,(NX + 1) * NZ * NY * sizeof(dcomplex));
    if(kt == 3)
	exit(0);
