...
int main(int argc,char *argv[])
{
  ...
  it = 1;
  if(restart_id){
	read_cp(rank,&it,&location,u,NR * sizeof(double));
	read_cp(rank,&it,&location,r,NR * sizeof(double));
  }
  for (it; it <= nit; it++) {
    ...
    resid(u, v, r, n1, n2, n3, a, k);
    if (timeron) timer_stop(T_resid2);
    write_cp(rank,it,u,NR * sizeof(double));
    write_cp(rank,it,r,NR * sizeof(double));
    if(it == 3)
	    exit(0);
  }
