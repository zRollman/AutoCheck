...
int main(int argc,char *argv[]) 
{
  ...
  k = 1;
  if(restart_id){
	  read_cp(rank,&k,&location,&sy,1 * sizeof(double));
	  read_cp(rank,&k,&location,&sx,1 * sizeof(double));
	  read_cp(rank,&k,&location,q,NQ * sizeof(double));
	  //read_cp(rank,&k,&location,x,2 * NK * sizeof(double));
  }
  ...
  for (k; k <= np; k++) {
    ...
    write_cp(rank,k,&sy,1 * sizeof(double));
    write_cp(rank,k,&sx,1 * sizeof(double));
    write_cp(rank,k,q,NQ * sizeof(double));
    //write_cp(rank,k,x,NK * 2 * sizeof(double));
    if(k == 3)
	    exit(0);
